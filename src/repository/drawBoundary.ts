...
export class DrawBoundaryFactory {
    ...
    draw() {
        ...
        // get upper points which moves in right/clockWise
        for (let i = 0; i < points.length; i++) {
            const point = points[i];
            while (upper.length >= 2 && this.isNotRightTurn(upper[upper.length - 2], upper[upper.length - 1], point)) {
                upper.pop();
            }
            upper.push(point);
        }

        // get lower points which moves in right/clockWise
        for (let i = points.length - 1; i >= 0; i--) {
            const point = points[i];
            while (lower.length >= 2 && this.isNotRightTurn(lower[lower.length - 2], lower[lower.length - 1], point)) {
                lower.pop();
            }
            lower.push(point);
        }

        // Combine upper and lower which is a list outermost point
        const outline = [...upper, ...lower];

        // Find and insert a mid point between a slope
    ...
    isNotRightTurn(a: any, b: any, c: any) {
